...
namespace lpr
{
	...
	CharsIdentify::CharsIdentify() 
	{
		...
		annChinese_ = cv::Algorithm::load<cv::ml::ANN_MLP>(kChineseAnnPath);
		ann_ = cv::Algorithm::load<cv::ml::ANN_MLP>(kDefaultAnnPath);
	}
	...
	void CharsIdentify::LoadModel(std::string path) {
		if (path != std::string(kDefaultAnnPath)) {
			...
			ann_ = cv::Algorithm::load<cv::ml::ANN_MLP>(path);
		}
	}
	...
	void CharsIdentify::LoadChineseModel(std::string path) {
		if (path != std::string(kChineseAnnPath)) {
			...
			annChinese_ = cv::Algorithm::load<cv::ml::ANN_MLP>(path);
		}
	}
